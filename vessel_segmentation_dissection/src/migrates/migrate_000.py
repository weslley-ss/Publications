...
def merge_configs(old_path: Path, new_path: Path, CHECK_KEYS, EXCLUDED_KEYS, identificador = None) -> bool:
    if old_path.exists() and new_path.exists():
        ...
        print(f"File not found | {identificador} on Old: {old_path.exists()} or New: {new_path.exists()}")
        return False
...
def main():
    ...
    # !! AJUSTAR MIGRATE 000 PARA CORRIGIR CONFIGURAÇÕES DE TREINAMENTO E MODELO,
    configs = np.arange(0, 181, dtype=int)
    data_sets = ["DRIVE", "VessMAP"]
    
    
    for data_set in data_sets:
        
        base_old = Path(f"/home/weslley/Documentos/Experimentos-baseline/vessel_baseline/experiments_{data_set}")
        base_new = Path(f"/home/weslley/Documentos/Publications/vessel_segmentation_dissection/experiments/{data_set}/baseline_transform/dic_unet/baseline_variation")
        ...
        for seed in [2,4,8]: 
            for index in configs:
                    ...
                    merge_configs(old_config_path, new_config_path, 
                                ...
                                EXCLUDED_KEYS=["epochs", "transform", "variation", "model_id","save_path"], identificador=f"CONFIG {index:03d}_{seed}")
                    ...
                    merge_metrics(old_metrics_path, new_metrics_path, identificador=f"Json {index:03d}_{seed}")            
